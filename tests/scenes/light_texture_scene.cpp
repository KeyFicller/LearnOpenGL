...
light_texture_scene::light_texture_scene()
    : camera_scene_base("Light Texture Test"), m_shader(nullptr) {}
...
void light_texture_scene::init(GLFWwindow *_window) {
  ...
  // Set vertex attributes (position: 3 floats, normal: 3 floats, texcoord: 2
  // floats)
  ...
  // Load shader
  try {
    m_shader = new shader("shaders/light_texture_test/vertex.shader",
                          "shaders/light_texture_test/fragment.shader");
    m_light_shader =
        new shader("shaders/light_texture_test/vertex.shader",
...
void light_texture_scene::render() {
  ...
  light_model = glm::translate(light_model, m_light.m_position);
